...
// Tiempo de reloj real (wall-clock) en segundos.
...
// Tiempo de CPU del proceso (usuario y kernel) en segundos.
...
static void usage(const char *p) {
	fprintf(stderr, "Uso: %s N trials threads [seed]\n", p);
	fprintf(stderr, "Ej:  %s 400 10 4\n", p);
	fprintf(stderr, "Ej:  %s 1000 10 8 123456789\n", p);
}
...
static void zero_matrix(int N, int32_t *C) {
	...
	#pragma omp parallel for schedule(static) // Paraleliza el llenado de ceros en la matriz C
...
// ============================================================
//  MULTIPLICACION PARALELA (OpenMP)
// ============================================================

// C = A x B usando OpenMP en el espacio (i, j).
...
int main(int argc, char **argv) {
	...
	if (!parse_int(argv[3], &threads)) { usage(argv[0]); return 1; }
	if (argc >= 5) seed = (unsigned)strtoul(argv[4], NULL, 10);

	// Fija el numero de hilos solicitado por linea de comandos.
	...
	// Warm-up para estabilizar la primera medicion.
	zero_matrix(N, C);
	matmul_omp(N, A, B, C);

	// Formato de salida:
	// N trial threads wall user kernel cpu_total checksum seed
