...
def read_speedup_row(csv_path: Path) -> tuple[list[str], dict[str, float]]:
    ...
    speedup_values: list[str] | None = None
    for row in rows[1:]:
        if not row:
            continue

        first = row[0].strip().lower()
        if first.startswith("speedup"):
            speedup_values = row
            break

    if speedup_values is None:
        raise ValueError(f"No se encontro la fila de speedup en: {csv_path}")

    matrix_sizes = headers[1:]
    if len(speedup_values) < len(headers):
        raise ValueError(f"Fila speedup incompleta en: {csv_path}")

    speedup_by_size: dict[str, float] = {}
    for i, size in enumerate(matrix_sizes, start=1):
        speedup_by_size[size] = float(speedup_values[i])

    return matrix_sizes, speedup_by_size
...
def write_summary_png() -> None:
    ...
    plt.title("Tabla resumen de speedup por prueba", fontsize=14, pad=18)
    fig.tight_layout()
...
def main() -> None:
    ...
    for label, csv_path in SOURCES:
        sizes, speedup_by_size = read_speedup_row(csv_path)
        ...
        row = [label] + [f"{speedup_by_size[size]:.6f}" for size in expected_sizes]
