...
THREADS_FILE = BASE_DIR / "output_hilos_openmp.txt"
OUTPUT_FIGURE = BASE_DIR / "speedup_hilos_openmp_vs_secuencial.png"
...
def main() -> None:
    ...
    if not common_sizes:
        raise ValueError(
            "No hay tamanos N en comun entre la corrida secuencial y la de OpenMP."
        )
    ...
    for thread_count in THREAD_COUNTS:
        ...
        valid_positions = [common_sizes.index(n_value) for n_value in valid_sizes]
        speedup = np.array(
            [
        ...
        ax.plot(
            ...
            marker="o",
            linewidth=2,
            label=f"{thread_count} hilos OpenMP",
        )

    ax.set_title("Speedup de hilos OpenMP respecto a secuencial")
    ...
    print("\nPromedios por numero de hilos OpenMP (wall_s):")
    for thread_count in THREAD_COUNTS:
        print(f"\n{thread_count} hilos")
        for n_value in common_sizes:
            if thread_count in thread_times[n_value]:
