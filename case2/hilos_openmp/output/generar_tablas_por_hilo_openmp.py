...
OPENMP_INPUT = BASE_DIR / "output_hilos_openmp.txt"
SECUENCIAL_INPUT = BASE_DIR.parent.parent / "secuencial" / "output" / "times_secuencial.txt"
THREAD_COUNTS = (2, 4, 8, 16)


def load_openmp_wall_times(path: Path) -> dict[int, dict[int, list[float]]]:
...
def compute_speedups(
    ...
    for n_value in sizes:
        ...
        speedups[n_value] = base / openmp_means[n_value]
...
def write_csv(
    ...
    out_csv = BASE_DIR / f"tabla_hilos_openmp_{thread_count}.csv"
...
def write_png(thread_count: int, csv_path: Path) -> Path:
    out_png = BASE_DIR / f"tabla_hilos_openmp_{thread_count}.png"
    ...
    plt.title(f"Tiempos por ronda - {thread_count} hilos OpenMP", fontsize=13, pad=16)
    fig.tight_layout()
...
def main() -> None:
    if not OPENMP_INPUT.exists():
        raise FileNotFoundError(f"No existe archivo de OpenMP: {OPENMP_INPUT}")

    if not SECUENCIAL_INPUT.exists():
        raise FileNotFoundError(f"No existe archivo secuencial: {SECUENCIAL_INPUT}")

    openmp_data = load_openmp_wall_times(OPENMP_INPUT)
    secuencial_means = load_sequential_means(SECUENCIAL_INPUT)

    for thread_count in THREAD_COUNTS:
        by_n = openmp_data.get(thread_count)
        if not by_n:
            print(f"Sin datos para {thread_count} hilos OpenMP, se omite")
            continue
