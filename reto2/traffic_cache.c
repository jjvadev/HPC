 ...
 * ---------------------------------------------------------------- */
static void init_road(uint8_t *road, int N, double density) {
    for (int i = 1; i <= N; i++) {
 ...
 * ---------------------------------------------------------------- */
static int count_cars(const uint8_t *road, int N) {
    int total = 0;
    for (int i = 1; i <= N; i++) {
        total += road[i];
    }
 ...
 * ---------------------------------------------------------------- */
static int update_road(const uint8_t *old, uint8_t *new_road, int N) {
    int moved = 0;

    
    for (int i = 1; i <= N; i++) {
        uint8_t curr = old[i];
        uint8_t right = old[i+1];
        uint8_t left = old[i-1];
        if (curr == 1 && right == 0) {
...
int main(int argc, char *argv[]) {
    ...
    /* --- Reservar memoria --- */
    uint8_t *road_a = (uint8_t *)malloc((N + 2) * sizeof(uint8_t));
    uint8_t *road_b = (uint8_t *)malloc((N + 2) * sizeof(uint8_t));
